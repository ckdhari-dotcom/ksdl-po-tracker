...
async function autoFillUploadedPo_(file) { if (!file) return; try { toast('Reading PO copy…'); const text = file.type === 'application/pdf' || /\.pdf$/i.test(file.name) ? await textFromUploadedPdf_(file) : await ocrUploadedImage_(file); const parsed = parseUploadedPo_(text); const form = $('poForm').elements; if (parsed.customerName && !form.customerName.value) form.customerName.value = parsed.customerName; if (parsed.poNumber && !form.poNumber.value) form.poNumber.value = parsed.poNumber; if (parsed.poDate && !form.poDate.value) form.poDate.value = parsed.poDate; if (parsed.deliveryDate && !form.deliveryDate.value) form.deliveryDate.value = parsed.deliveryDate; if (parsed.poValue !== '' && !form.poValue.value) form.poValue.value = parsed.poValue; if (!form.remarks.value) form.remarks.value = `PO copy uploaded: ${file.name}`; const captured = [parsed.poNumber, parsed.poDate, parsed.deliveryDate, parsed.poValue].filter(value => value !== '').length; toast(captured ? 'PO details filled — please verify, then save' : 'PO copy saved — please enter the details manually'); } catch (error) { toast('Could not read the PO copy — please enter details manually'); } }
...
render = function() {
  ...
  [...$('poTableBody').rows].forEach((row, index) => {
    ...
    const locationCell = row.insertCell(row.cells.length - 1);
