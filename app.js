...
function uploadedPoIsoDate_(value) { const match = String(value || '').match(/^(\d{2})[.\/-](\d{2})[.\/-](\d{4})$/); return match ? `${match[3]}-${match[2]}-${match[1]}` : ''; }
function normaliseUploadedPoText_(text) { return String(text || '').replace(/\u00a0/g, ' ').replace(/\s+/g, ' ').trim(); }
function uploadedDeliveryLocation_(text) { const match = text.match(/Ship\s*To\s+Avenue\s+Supermarts\s+Ltd\.?\s+(.{2,80}?)\s+DMart\b/i); return match ? match[1].trim() : ''; }
function parseUploadedPo_(rawText) { const text = normaliseUploadedPoText_(rawText); const grouped = text.match(/PO\s*#\s*PO\s*Date\s*Delivery\s*(?:Dt|Date)\s*(\d{6,})\s*(\d{2}[.\/-]\d{2}[.\/-]\d{4})\s*(\d{2}[.\/-]\d{2}[.\/-]\d{4})/i); const dates = text.match(/\b\d{2}[.\/-]\d{2}[.\/-]\d{4}\b/g) || []; const poMatch = text.match(/PO\s*(?:#|No\.?|Number)?\s*:?\s*(\d{6,})/i); const totalMatch = text.match(/\bTotal\s+\d+(?:\.\d+)?\s+([\d,]+\.\d{1,2})\b/i) || text.match(/(?:Grand\s+)?Total\s*:?\s*₹?\s*([\d,]+\.\d{1,2})/i); return {customerName: /dmart|avenue\s+supermarts/i.test(text) ? 'DMart' : '', poNumber: grouped?.[1] || poMatch?.[1] || '', deliveryLocation: uploadedDeliveryLocation_(text), poDate: uploadedPoIsoDate_(grouped?.[2] || dates[0]), deliveryDate: uploadedPoIsoDate_(grouped?.[3] || dates[1]), poValue: totalMatch ? Number(totalMatch[1].replace(/,/g, '')) : ''}; }
async function ocrUploadedImage_(source) { const result = await Tesseract.recognize(source, 'eng', {logger: message => { if (message.status === 'recognizing text' && message.progress > 0.65) toast('Reading PO copy…'); }}); return result.data.text || ''; }
async function textFromUploadedPdf_(file) { if (!window.pdfjsLib) throw new Error('PDF reader could not load'); window.pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js'; const pdf = await window.pdfjsLib.getDocument({data: await file.arrayBuffer()}).promise; let text = ''; for (let number = 1; number <= Math.min(pdf.numPages, 3); number++) { const page = await pdf.getPage(number); const content = await page.getTextContent(); text += ` ${content.items.map(item => item.str).join(' ')}`; } if (text.trim().length >= 80) return text; for (let number = 1; number <= Math.min(pdf.numPages, 2); number++) { const page = await pdf.getPage(number); const viewport = page.getViewport({scale: 2}); const canvas = document.createElement('canvas'); canvas.width = viewport.width; canvas.height = viewport.height; await page.render({canvasContext: canvas.getContext('2d'), viewport}).promise; text += ` ${await ocrUploadedImage_(canvas)}`; } return text; }
async function autoFillUploadedPo_(file) { if (!file) return; try { toast('Reading PO copy…'); const text = file.type === 'application/pdf' || /\.pdf$/i.test(file.name) ? await textFromUploadedPdf_(file) : await ocrUploadedImage_(file); const parsed = parseUploadedPo_(text); const form = $('poForm').elements; if (parsed.customerName && !form.customerName.value) form.customerName.value = parsed.customerName; if (parsed.poNumber && !form.poNumber.value) form.poNumber.value = parsed.poNumber; if (parsed.deliveryLocation && !form.deliveryLocation.value) form.deliveryLocation.value = parsed.deliveryLocation; if (parsed.poDate && !form.poDate.value) form.poDate.value = parsed.poDate; if (parsed.deliveryDate && !form.deliveryDate.value) form.deliveryDate.value = parsed.deliveryDate; if (parsed.poValue !== '' && !form.poValue.value) form.poValue.value = parsed.poValue; if (!form.remarks.value) form.remarks.value = `PO copy uploaded: ${file.name}`; const captured = [parsed.poNumber, parsed.deliveryLocation, parsed.poDate, parsed.deliveryDate, parsed.poValue].filter(value => value !== '').length; toast(captured ? 'PO details filled — please verify, then save' : 'PO copy saved — please enter the details manually'); } catch (error) { toast('Could not read the PO copy — please enter details manually'); } }
...
function initialiseManualPoChoice_() {
  ...
  $('poForm').elements.poAttachment.addEventListener('change', event => {
    autoFillUploadedPo_(event.target.files?.[0]);
  });
